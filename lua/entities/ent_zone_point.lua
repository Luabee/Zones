...
local LocalPlayer, Vector, GetConVarNumber, Material, IsValid, render, cam, table, Color = LocalPlayer, Vector, GetConVarNumber, Material, IsValid, render, cam, table, Color
	...
	if wep:IsValid() and wep:GetClass() == "weapon_zone_designator" then
		...
		if wep:GetZoneClass() == self:GetZoneClass() or GetConVarNumber("zone_filter") == 0 then
			...
			if IsValid(next) then
				...
				local col1 = zones.Classes[class].Color
				col1.a = 80
				local col2 = {a=80}
				col2.r = col1.r * .5
				col2.g = col1.g * .5
